// Version 11.0 (from ver 10.0)
// Add seperate image in the chat bubble
...
// A helper to load classification rules from JSON
function loadClassificationRules() {
  const filePath = path.join(__dirname, "classification_rules.json");
  const rawData = fs.readFileSync(filePath, "utf8");
  return JSON.parse(rawData);
}

function generateSystemPrompt(categories) {
  /*
    Something like:

    You are a helpful medical triage assistant. 
    Your job is to read a user's symptom description and classify it into exactly one of the categories:

    1) 'abdominal pain'
       - Examples / synonyms: "肚子痛", "胃痛", "腹痛"

    2) 'ringing in the ears (tinnitus)'
       - Examples / synonyms: "耳朵有嗡嗡聲", "我耳朵一直在叫", ...
  ...
  */
  ...
  categories.forEach((cat, index) => {
    const idx = index + 1;
    prompt += `${idx}) '${cat.label}'\n`;
    if (cat.synonyms && cat.synonyms.length > 0) {
      prompt += `   - Examples / synonyms: ${cat.synonyms.join(", ")}\n`;
    }
    prompt += "\n";
  });

  prompt += "Return ONLY the most relevant category name, exactly as it appears in the list above.\n";
  prompt += "No extra text. No disclaimers.\n";
...
async function classifySymptomHandler(userInput) {
  // 1. Load rules and build system prompt
  const classificationRules = loadClassificationRules();
  const systemPrompt = generateSystemPrompt(classificationRules.categories);
  ...
  try {
    ...
    // 3. Match the reply to our categories
    for (const { label, trigger, node } of classificationRules.categories) {
      if (Array.isArray(trigger)) {
        // If trigger is an array, check if any element is included
        if (trigger.some((t) => chatgptReply.includes(t))) {
          return node;
        }
      } else {
        // Otherwise, treat trigger as a string
        if (chatgptReply.includes(trigger)) {
          return node;
        }
      }
    }

    // 4. If no match found, fallback
...
async function askQuestion(currentNodeId, userRawMessage = "", user_id = "", userLowerCase = "") {
  ...
  let nextNodeId = currentNodeId;         // default to current
  let finalBotReply = botReply;           // default to the node's question
  let chosenLabel = null;                 // for multiple_choice

  switch (node.type) {
    case "end":
      // Terminal node
      nextNodeId = "end";  // no real next node
      finalBotReply = botReply;
      break;
      ...
      case "function": {
        ...
        if (handlerName === "classifySymptomHandler") {
          ...
          const targetNodeId = await classifySymptomHandler(userRawMessage);
          // 2) Build next question
          const nextNodeObj = getNode(targetNodeId) || {};
          let nextQuestion = nextNodeObj.question || "";
          if (nextQuestion.includes("{{SYMPTOM}}")) {
            nextQuestion = nextQuestion.replace("{{SYMPTOM}}", userRawMessage);
          }
          // Assign
          nextNodeId = targetNodeId;
          finalBotReply = nextQuestion;
        } else {
          nextNodeId = "end";
          finalBotReply = "該功能無法使用。";
        }
        break;
      }
    ...
    case "open-ended": {
      ...
      if (currentNodeId === "get_duration") {
        const rawDuration = userRawMessage.trim(); // "3天" or "1週" etc.
        // You could do some basic validation or parsing:
        const durationRegex = /^(\d+)(天|週|月|年|小時)$/;
        if (!durationRegex.test(rawDuration)) {
          // If invalid, prompt user again
          nextNodeId = currentNodeId;
          finalBotReply = "格式不正確，請輸入像 2小時, 3天, 1週, 2月 這樣的格式。";
        } else {
          // Move on
          nextNodeId = node.next || "end";
          const nextNodeObj = getNode(nextNodeId) || {};
          finalBotReply = nextNodeObj.question || "";
        }
      
        // If valid, store it
        conversationState[user_id].answers = conversationState[user_id].answers || {};
        conversationState[user_id].answers[currentNodeId] = {
      ...
      if (currentNodeId === "get_pasthistory") {
        // 2a. Store the user’s response in conversationState
        conversationState[user_id].answers = conversationState[user_id].answers || {};
        conversationState[user_id].answers[currentNodeId] = {
          question: node.question,
          answer: userRawMessage,
        };
    
        // 2b. Move on to the next node
        const nextNodeId = node.next || "end";
        const nextNode = getNode(nextNodeId) || {};
        const nextQuestion = nextNode.question || "";
        return [nextNodeId, nextQuestion];
    ...
    case "yes_no": {
      ...
      if (yesList.includes(userLowerCase)) {
        nextNodeId = node.nextIfYes || "end";
        const nextObj = getNode(nextNodeId) || {};
        finalBotReply = nextObj.question || "";
      } else if (noList.includes(userLowerCase)) {
        nextNodeId = node.nextIfNo || "end";
        const nextObj = getNode(nextNodeId) || {};
        finalBotReply = nextObj.question || "";
      } else {
        // Invalid => remain on same node
        nextNodeId = currentNodeId;
        finalBotReply = "請回答是或否。";
      }
      break;
    }

    case "multiple_choice": {
      if (node.options && node.options.includes(userLowerCase)) {
        // valid choice
        chosenLabel = node.optionLabels?.[userLowerCase] || userRawMessage;
        nextNodeId = node.next[userLowerCase];
        const nextObj = getNode(nextNodeId) || {};
        finalBotReply = nextObj.question || "";
      } else {
        // Invalid => remain on same node
        nextNodeId = currentNodeId;
        finalBotReply = "無效的選項，請選擇有效的選項。";
      }
      break;
    }
  ...
  const nextNode = getNode(nextNodeId);
  const nodeImage = nextNode?.image || null;

  return [nextNodeId, finalBotReply, chosenLabel, nodeImage];
}
...
app.post("/chatbot", async (req, res) => {
  ...
  try {
    ...
    const oldQuestion = node ? node.question : "";
    const nodeNumber = node ? node.No : "";
    const specialtyValue = node?.meta?.specialty || "";

    // 2) Ask the question, but pass both the raw and lowercased text
    //    so you can handle placeholders + classification in askQuestion.
    const [nextNode, botReply, chosenLabel, nodeImage] = await askQuestion(
      currentNode, 
...
if (user_id && message) {
  console.log("Question to be saved:", finalQuestion);
  console.log("Message to be saved:", message);
  // The "botReply" (or "node.question") is the question asked by the bot
    ...
    .insert([
      {
        ...
        question: oldQuestion,
        message: message,      // the user's answer
        // you could also store additional fields, e.g. created_at, if needed
      },
    ]);
