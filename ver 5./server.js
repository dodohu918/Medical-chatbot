// Version 5.0
// Improved the code of requesting classification from openai to be more compact
...
// Build a prompt listing the exact category labels
  ...
  try {
    ...
    for (const { label, trigger, node } of classificationRules.categories) {
      if (chatgptReply.includes(trigger)) {
        return node; // Return this node if matched
      }
    }
