// Version 3
// This version contains the ability to provide user with the url to the clinics page!
...
const flowRLSData   = fs.readFileSync("flows/flow_2lowerLimbWeirdFeeling.json", "utf8");
const flowNeckMassData   = fs.readFileSync("flows/flow_neckMass.json", "utf8");
...
const flowRLS   = JSON.parse(flowRLSData);
const flowNeckMass   = JSON.parse(flowNeckMassData);
 ...
 */
async function askQuestion(currentNodeId, userMessage = "", user_id = "") {
  ...
  switch (node.type) {
    ...
    case "open-ended": {
      ...
      if (currentNodeId === "greeting" && userMessage.trim()) {
        ...
        const nextNodeId = node.next || "end";
        return await askQuestion(nextNodeId, userMessage, user_id);
      } else {
    ...
    case "multiple_choice": {
      ...
      if (node.options && node.options.includes(lowerMsg)) {
        // valid choice
        const chosenLabel = node.optionLabels?.[lowerMsg] || userMessage;
