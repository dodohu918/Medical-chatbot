// Version 6.0
// Add the past history and medical history open question part
// Improved the function generateSystemPrompt(categories) part so that it takes the "synonyms" in the classification_rules.json
// Add the "synonyms" part in classification_rules.json so that if users type in any weird shit as chief complaint, the classifySymptomHandler will always able to catch that
//   In the future, if any chief complaint that is hard for the algorithm to classify, we can just add that in the "synonyms"
// On the other hand, the "trigger" part in the classification_rules.json is only for better catch the string returned from chatgpt after classification
